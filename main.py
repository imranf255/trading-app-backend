...
def download_history(user_id: int, db: Session = Depends(get_db)):
    ...
    for ticker, pos in current_positions.items():
        if pos["shares"] > 0:
            ...
            try:
                stock = yf.Ticker(ticker)
                current_price = stock.history(period="1d")["Close"].iloc[-1] if not stock.history(period="1d").empty else avg_price
            except:
...
# [Include all other endpoints from original file: register, login, market-status, price, history, news, earnings, buy, sell, portfolio, trade-history, user-stats, leaderboard, limit-order, portfolio-analytics, alerts, day-trades, market-hours-config, check-limit-orders]
...
def get_price(ticker: str):
    try:
        stock = yf.Ticker(ticker)
        current_price = stock.history(period="1d")["Close"].iloc[-1] if is_market_open() else get_fallback_price(ticker)
...
def get_history(ticker: str, period: str = "1d"):
    try:
        ...
        return {"ticker": ticker, "times": [t.strftime("%H:%M") for t in hist.index], "prices": hist["Close"].tolist()}
    except:
        ...
        for item in news:
            ...
            if title and len(title) > 5:
                formatted_news.append({
                    ...
                    "published": datetime.fromtimestamp(item.get("providerPublishTime", 0)).strftime("%b %d, %Y")
                })
...
def buy_stock(user_id: int, request: TradeRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        current_price = stock.history(period="1d")["Close"].iloc[-1] if is_market_open() else get_fallback_price(request.ticker)
        if current_price is None:
            raise HTTPException(status_code=400, detail=f"Cannot fetch price for {request.ticker}")
    except:
